...
public class Float : MonoBehaviour {
	...
	void Update()
	{
		...
		if (angle > 360) angle -= 360;
			transform.position = new Vector3(transform.position.x, transform.position.y + maxUpAndDown * Mathf.Sin(angle * toDegrees), transform.position.z); //transform.position.y = maxUpAndDown * Mathf.Sin(angle * toDegrees);
	}
